		...
		if frappe.db.get_value("Buying Settings", None, "po_required") == "Yes":
			...
			for d in self.get("items"):
				if not d.purchase_order:
					item_cd=frappe.db.get_value("Item",d.item_code,"custom_allow_purchase_invoice_creation_without_purchase_order")
					if item_cd==0:
		...
		if frappe.db.get_value("Buying Settings", None, "pr_required") == "Yes":
			...
			for d in self.get("items"):
				if not d.purchase_receipt and d.item_code in stock_items:
					item_cd=frappe.db.get_value("Item",d.item_code,"custom_allow_purchase_invoice_creation_without_purchase_receipt")
					if item_cd==0:
